import math, os, json, threading, sys, socket, io, datetime
...
def create_file(name):
    ...
    if (e != ""):
        ...
        if chdir:
            ...
            print("file named '", name, "' already exists!")
            return
...
def create_directory(name):
    ...
    if (chdir):
        ...
        o[name] = {}
        print("Directory named '" + name + "' created successfully")
...
def mov_file(source, destination):
    print("current path when moving >>> ", current)
    ...
    o[new_name] = x
    print("File '" + fileName + "' moved successfully to '" + destination + "'!")
...
def open_file(name, mode="r"):
    ...
    if (mode not in ["r", "w", "rw"]):
        ...
        if chdir:
            ...
            print("File named " + name + " opened successfully in " + mode + " mode!")
        else:
...
def close(name):
    ...
    for key in list(openfile):
        if name == openfile[key]['filename'] and CURRENT_DIR == openfile[key]["path"]:
            wasOpen = True
            openfile.pop(key)
            print("File named " + name + " closed successfully.")
    if (not wasOpen):
        print("file is already closed or does not exist in the current directory!")
                ...
                for j in range(MAX_BLOCKS):
                    if blocks[j]["occupied"]:
                        ...
                        if data_left < BLOCK_SIZE:
                            ...
                            prev_bloc_idx = j
                            break
        print("Content '", data, "' written to file '", name, "' successfully!")
    else:
        if not wasOpen:
            print(
                "File is not opened or exists in some other directory, change directory to that of file to perform write!")
            pwd()
        elif mode != "write":
...
def write_to_file(name, pos, data):
    ...
    if (wasOpen and mode == "write"):
        ...
        print("Content '", data, "' written to file '", name, "' at position", pos, "successfully!")
    else:
        if not wasOpen:
            print(
                "File is not opened or exists in some other directory, change directory to that of file to perform write!")
...
def read(filename):
    ...
    if (wasOpen and mode == "read"):
        ...
        if not wasOpen:
            print(
                "File is not opened or exists in some other directory, change directory to that of file to perform read!")
            pwd()
        elif mode != "read":
...
def read_size(filename, start, size):
    ...
    if (wasOpen and mode == "read"):
        ...
        if not wasOpen:
            print(
                "File is  not opened or exists in some other directory, change directory to that of file to perform read from pos!")
            pwd()
        elif mode != "read":
            print("File is  not opened in read mode")
...
def truncate(filename, path_to_file, size):
    ...
    chdir = checkKey(o, filename)
    if not chdir:
        print("File '" + filename + "' not present at the path specified!")
        return
    ...
    print("File '", filename, "' truncated successfully!")
...
def delete(filename, path_to_file):
    ...
    chdir = checkKey(o, filename)
    if not chdir:
        print("File '" + filename + "' not found in the directory '" + path_to_file + "' hence not deleted!")
        return
...
def memory_map(directory_structure=dir_str, spacing=0, file=None):
    # accessing the directory structure
    ...
    for dir in o:
        ...
        if e == "":
            ...
            if file != None:
                file.write("\t" * spacing, "Directory: ", dir, sep="\t")
            memory_map(o[dir], spacing + 1)
        else:
            # if its a file
            blocks_of_file = []
            # "start_block" is at index 0
            ...
            if file != None:
                file.write("\t" * spacing, "File: ", dir, blocks_of_file, "File Size: ", str(o[dir]["size"]) + " bytes",
                           "Starting Address: " + str(starting_address), sep="\t")
...
def client_thread(client, addr):
    print('The new client has socket id:', addr)
    while True:
        try:
            command = client.recv(512).decode()
            if command == 'bye':
                print("Connection closed successfully '" + command + "' command", 'from:', addr)
                break
            if command == 'Error Occurred, closing connection':
                print('command:', command, 'from:', addr)
                break
            
            print('command:', command, 'from:', addr)
            sys.stdout = io.StringIO() # set stdout
            exec(command)  # execute command
            output = sys.stdout.getvalue()
            client.sendall(output.encode())   # send output
            ...
            print('command:', command, 'from:', addr)
            client.sendall("Error Occurred, closing connection".encode())
            break
    store_data()
...
sock.listen()
while True:
    print('Listening for the incoming clients')
    client, addr = sock.accept()
    client.sendall('Successfully Connected'.encode())

    'Start the new client thread'
    threading._start_new_thread(client_thread, (client, addr))

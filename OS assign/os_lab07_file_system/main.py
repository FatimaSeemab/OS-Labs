...
def write(name, data):
    ...
    if wasOpen and mode == "write":
        ...
        elif block_data_length < 16:
            #checking space in starting block
            space_in_block = int(16 - block_data_length)
            blocks[block_index]['data'] = blocks[block_index]['data'] + data[0:space_in_block]
            print("starting index of data,"0)
            print("stopping index of data,"space_in_block)
                ...
                for j in range(MAX_BLOCKS):
                    if blocks[j]["occupied"]:
                        ...
                        if data_left < 16:
                                ...
                                stop_idx = starting_idx + 15
                                blocks[j]['data'] = data[starting_idx:stop_idx]
                                blocks[prev_bloc_idx]['next'] = j
                                starting_idx = stop_idx+1
                                prev_bloc_idx = j
                                break;
        else:
                ...
                for j in range(MAX_BLOCKS):
                    if blocks[j]["occupied"]:
                        continue;
                         ...
                         if data_left < 16:
                            blocks[j]['data'] = data[0:starting_idx+data_left]
                            blocks[prev_bloc_idx]['next'] = j
                            break
                         else:
...
def read(filename):
    ...
    if (wasOpen and mode == "read"):
        ...
        while block_idx != "None":
            buffer=buffer+ blocks[block_idx]['data']
...
write_to_file("file.txt","Hi i am fatima")
close("file.txt")
open("file.txt", "r")
# write("file.txt",
